...
class getid3_id3v1
{
	...
	function __construct(&$fd, &$ThisFileInfo) {
		...
		if (substr($id3v1tag, 0, 3) == 'TAG') {
			...
			$ParsedID3v1['genre'] = getid3_id3v1::LookupGenreName($ParsedID3v1['genreid']);
			...
			$GoodFormatID3v1tag = $this->GenerateID3v1Tag(
											...
											$ParsedID3v1['album'],
											$ParsedID3v1['year'],
											(isset($ParsedID3v1['genre']) ? getid3_id3v1::LookupGenreID($ParsedID3v1['genre']) : false),
											$ParsedID3v1['comment'],
											@$ParsedID3v1['track']);
	...
	public function cutfield($str) {
		return trim(substr($str, 0, strcspn($str, "\x00")));
	}

	static public function ArrayOfGenres($allowSCMPXextended=false) {
	...
	static public function LookupGenreName($genreid, $allowSCMPXextended=true) {
		switch ($genreid) {
			case 'RX':
			case 'CR':
				break;
	...
	static public function LookupGenreID($genre, $allowSCMPXextended=false) {
		$GenreLookup = getid3_id3v1::ArrayOfGenres($allowSCMPXextended);
	...
	static public function StandardiseID3v1GenreName($OriginalGenre) {
	...
	public function GenerateID3v1Tag($title, $artist, $album, $year, $genreid, $comment, $track='') {
		$ID3v1Tag = 'TAG';
