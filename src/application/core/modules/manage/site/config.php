    ...
    private function _save()
    {
        ...
        foreach ($this->config_groups as $group) {
            foreach ($group->options as $option) {
                if ('true_false' == $option->config_type) {
                    $_POST['config'][$option->config_variable] = (bool) $_POST['config'][$option->config_variable];
                }
            }
        }
        // echo '<pre>';
        ...
        if (false !== file_put_contents(KX_ROOT.'/config/main.yml.php', $new_config)) {
