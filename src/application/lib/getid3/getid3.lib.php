...
class getid3_lib
{
	...
	static public function PrintHexBytes($string, $hex=true, $spaces=true, $htmlsafe=true) {
		$returnstring = '';
	...
	static public function SafeStripSlashes($text) {
	...
	static public function trunc($floatnumber) {
	...
	static public function CastAsInt($floatnum) {
	...
	static public function DecimalBinary2Float($binarynumerator) {
	...
	static public function NormalizeBinaryPoint($binarypointnumber, $maxbits=52) {
	...
	static public function Float2BinaryDecimal($floatvalue) {
		// http://www.scri.fsu.edu/~jac/MAD3401/Backgrnd/binary.html
	...
	static public function Float2String($floatvalue, $bits) {
	...
	static public function LittleEndian2Float($byteword) {
		return getid3_lib::BigEndian2Float(strrev($byteword));
	}


	static public function BigEndian2Float($byteword) {
	...
	static public function BigEndian2Int($byteword, $synchsafe=false, $signed=false) {
		$intvalue = 0;
	...
	static public function LittleEndian2Int($byteword, $signed=false) {
		return getid3_lib::BigEndian2Int(strrev($byteword), false, $signed);
	}


	static public function BigEndian2Bin($byteword) {
		$binvalue = '';
		$bytewordlen = strlen($byteword);
		for ($i = 0; $i < $bytewordlen; $i++) {
	...
	static public function BigEndian2String($number, $minbytes=1, $synchsafe=false, $signed=false) {
		if ($number < 0) {
			return false;
		}
	...
	static public function Dec2Bin($number) {
		while ($number >= 256) {
	...
	static public function Bin2Dec($binstring, $signed=false) {
		$signmult = 1;
		if ($signed) {
			if ($binstring{0} == '1') {
				$signmult = -1;
	...
	static public function Bin2String($binstring) {
	...
	static public function LittleEndian2String($number, $minbytes=1, $synchsafe=false) {
		$intstring = '';
		while ($number > 0) {
			if ($synchsafe) {
	...
	static public function array_merge_clobber($array1, $array2) {
	...
	static public function array_merge_noclobber($array1, $array2) {
		if (!is_array($array1) || !is_array($array2)) {
			return false;
		}
		$newarray = $array1;
	...
	static public function fileextension($filename, $numextensions=1) {
		if (strstr($filename, '.')) {
	...
	static public function PlaytimeString($playtimeseconds) {
	...
	static public function image_type_to_mime_type($imagetypeid) {
	...
	static public function DateMac2Unix($macdate) {
	...
	static public function CreateDeepArray($ArrayPath, $Separator, $Value) {
	...
	static public function array_max($arraydata, $returnkey=false) {
		$maxvalue = false;
		$maxkey = false;
	...
	static public function array_min($arraydata, $returnkey=false) {
		$minvalue = false;
		$minkey = false;
		foreach ($arraydata as $key => $value) {
			if (!is_array($value)) {
	...
	static public function md5_file($file) {
	...
	static public function sha1_file($file) {
	...
	static public function hash_data($file, $offset, $end, $algorithm) {
		if ($end >= pow(2, 31)) {
			return false;
		}
	...
	static public function iconv_fallback_int_utf8($charval) {
		if ($charval < 128) {
			// 0bbbbbbb
	...
	// ISO-8859-1 => UTF-8
	static public function iconv_fallback_iso88591_utf8($string, $bom=false) {
	...
	// ISO-8859-1 => UTF-16BE
	static public function iconv_fallback_iso88591_utf16be($string, $bom=false) {
		$newcharstring = '';
		if ($bom) {
	...
	// ISO-8859-1 => UTF-16LE
	static public function iconv_fallback_iso88591_utf16le($string, $bom=false) {
		$newcharstring = '';
		if ($bom) {
	...
	static public function iconv_fallback_iso88591_utf16($string) {
		return getid3_lib::iconv_fallback_iso88591_utf16le($string, true);
	}

	// UTF-8 => ISO-8859-1
	static public function iconv_fallback_utf8_iso88591($string) {
	...
	// UTF-8 => UTF-16BE
	static public function iconv_fallback_utf8_utf16be($string, $bom=false) {
		$newcharstring = '';
		if ($bom) {
			$newcharstring .= "\xFE\xFF";
		}
	...
	// UTF-8 => UTF-16LE
	static public function iconv_fallback_utf8_utf16le($string, $bom=false) {
		$newcharstring = '';
		if ($bom) {
			$newcharstring .= "\xFF\xFE";
		}
	...
	// UTF-8 => UTF-16LE (BOM)
	static public function iconv_fallback_utf8_utf16($string) {
		return getid3_lib::iconv_fallback_utf8_utf16le($string, true);
	}

	// UTF-16BE => UTF-8
	static public function iconv_fallback_utf16be_utf8($string) {
	...
	// UTF-16LE => UTF-8
	static public function iconv_fallback_utf16le_utf8($string) {
	...
	// UTF-16BE => ISO-8859-1
	static public function iconv_fallback_utf16be_iso88591($string) {
		if (substr($string, 0, 2) == "\xFE\xFF") {
			// strip BOM
			$string = substr($string, 2);
		}
	...
	// UTF-16LE => ISO-8859-1
	static public function iconv_fallback_utf16le_iso88591($string) {
		if (substr($string, 0, 2) == "\xFF\xFE") {
			// strip BOM
			$string = substr($string, 2);
		}
	...
	static public function iconv_fallback_utf16_iso88591($string) {
	...
	// UTF-16 (BOM) => UTF-8
	static public function iconv_fallback_utf16_utf8($string) {
		$bom = substr($string, 0, 2);
		if ($bom == "\xFE\xFF") {
	...
	static public function iconv_fallback($in_charset, $out_charset, $string) {
	...
	static public function MultiByteCharString2HTML($string, $charset='ISO-8859-1') {
		$HTMLstring = '';
	...
	static public function RGADnameLookup($namecode) {
	...
	static public function RGADoriginatorLookup($originatorcode) {
	...
	static public function RGADadjustmentLookup($rawadjustment, $signbit) {
	...
	static public function RGADgainString($namecode, $originatorcode, $replaygain) {
		if ($replaygain < 0) {
			$signbit = '1';
		} else {
			$signbit = '0';
	...
	static public function RGADamplitude2dB($amplitude) {
		return 20 * log10($amplitude);
	}


	static public function GetDataImageSize($imgData, &$imageinfo) {
	...
	static public function ImageTypesLookup($imagetypeid) {
	...
	static public function CopyTagsToComments(&$ThisFileInfo) {
	...
	static public function EmbeddedLookup($key, $begin, $end, $file, $name) {
	...
	static public function IncludeDependency($filename, $sourcefile, $DieOnFailure=false) {
