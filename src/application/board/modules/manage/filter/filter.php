...
class manage_board_filter_filter extends kxCmd
{
    ...
    private function _postFilter()
    {
        ...
        if ('forever' == strtolower($this->request['filter_ban_duration'])) {
            // A ban lasting forever!
            $ban_duration = 0;
        } elseif ('' == $this->request['filter_ban_duration']) {
            $ban_duration = null;
        } elseif (!is_numeric($this->request['filter_ban_duration'])) {
        ...
        if (!isset($this->twigData['notice'])) {
            $this->twigData['notice']['message'] = ('add' == $this->request['do']) ? _('Filter successfully added!') : _('Filter successfully edited!');
