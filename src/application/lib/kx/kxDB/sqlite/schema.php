...
class DatabaseSchema_sqlite extends DatabaseSchema
{
    ...
    public function findTables($table_expression)
    {
        ...
        $result = db_query('SELECT name FROM '.$info['schema'].'.sqlite_master WHERE name LIKE :table_name', [
    ...
    protected function createFieldSql($name, $spec)
    {
        if (!empty($spec['auto_increment'])) {
            ...
            if (in_array($spec['sqlite_type'], ['VARCHAR', 'TEXT']) && isset($spec['length'])) {
                $sql .= '('.$spec['length'].')';
            }
    ...
    protected function introspectSchema($table)
    {
        ...
        foreach ($result as $row) {
            if (0 !== strpos($row->name, 'sqlite_autoindex_')) {
                $indexes[] = [
