...
class DatabaseSchema_pgsql extends DatabaseSchema
{
     ...
     *
     * This is collected by kxDB_pgsql->queryTableInformation(),
     * by introspecting the database.
     *
     * @see kxDB_pgsql->queryTableInformation()
     *
     * @var array
     */
    ...
    public function changeField($table, $field, $field_new, $spec, $new_keys = [])
    {
        ...
        if (in_array($spec['pgsql_type'], ['serial', 'bigserial', 'numeric'])) {
            $typecast = 'int';
        } else {
    ...
    protected function createFieldSql($name, $spec)
    {
        ...
        if (isset($spec['type']) && 'serial' == $spec['type']) {
            unset($spec['not null']);
        }
    ...
    protected function processField($field)
    {
        ...
        if (isset($field['type']) && 'serial' == $field['type']) {
