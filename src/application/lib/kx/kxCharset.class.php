...
class kxCharset
{
     ...
     *
     * @see http://php.net/manual/en/ref.iconv.php
     *
     * @param   string    Source string
     * @param   string    Source string char set
     * @param   string    Destination character set
     ...
     *
     * @see http://php.net/manual/en/ref.recode.php
     *
     * @param   string    Source string
     * @param   string    Source string char set
     * @param   string    Destination character set
     ...
     *
     * @see http://mikolajj.republika.pl/
     *
    ...
    protected function _convertCharsetTables($string, $sourceCharSet, $destCharSet)
    {
        ...
        if ('utf-8' != $sourceCharSet) {
            ...
            for ($i = 0; $i < strlen($string); ++$i) {
                ...
                // and solve it ;)
                if (('gsm0338' == $sourceCharSet) && ('1B' == $hexChar)) {
                    ++$i;
                    $hexChar .= strtoupper(dechex(ord($string[$i])));
                }
                ...
                if ((('big5' == $sourceCharSet || 'gb2312' == $sourceCharSet) && (ord($string[$i]) > 0x7F)) || ('cp936' == $sourceCharSet && ord($string[$i]) > 0x80)) {
                    ++$i;
                    $hexChar .= strtoupper(dechex(ord($string[$i])));
                }
                // SJIS has multibyte characters as well
                if (('shift_jis' == $sourceCharSet) && ((ord($string[$i]) > 0x7E) && !(ord($string[$i]) >= 0xA1 && ord($string[$i]) <= 0xDF))) {
                    ++$i;
                    $hexChar .= strtoupper(dechex(ord($string[$i])));
                }

                if ('utf-8' != $destCharSet) {
    ...
    protected function _unicodeEntity($unicodeString)
    {
        ...
        for ($charPosition = 0; $charPosition < $stringLenght; ++$charPosition) {
            ...
            if ($asciiChar < 128) { // 1 7 0bbbbbbb (127)
                $outString .= $char;
            } elseif (6 == $asciiChar >> 5) { // 2 11 110bbbbb 10bbbbbb (2047)
                ...
                $outString .= $entity;
            } elseif (14 == $asciiChar >> 4) { // 3 16 1110bbbb 10bbbbbb 10bbbbbb
                $firstByte = ($asciiChar & 31);
                ++$charPosition;
                $char = $unicodeString[$charPosition];
                $asciiChar = ord($char);
                ...
                $entity = sprintf('&#%d;', $asciiChar);
                $outString .= $entity;
            } elseif (30 == $asciiChar >> 3) { // 4 21 11110bbb 10bbbbbb 10bbbbbb 10bbbbbb
                $firstByte = ($asciiChar & 31);
                ++$charPosition;
                $char = $unicodeString[$charPosition];
                $asciiChar = ord($char);
    ...
    protected function _makeConvertTable($fromCharset, $toCharset = '')
    {
        ...
        for ($i = 0; $i < func_num_args(); ++$i) {
            ...
            $fileWithEncTabe = fopen($this->charsetPath.$fileName, 'r') or exit; // This die(); is just to make sure...
