...
class Upload
{
    ...
    public function HandleUpload($postData, $boardData)
    {
        ...
        if ($file_name[0]) {
            ...
            for ($i = 0; $i < $boardData->board_max_files; ++$i) {
                ...
                // Too big?
                if (0 == $i && array_sum($file_size) > $boardData->board_max_upload_size) {
                ...
                if ('.jpeg' == $this->files[$i]['file_type']) {
                ...
                if (isset($filetype_forcethumb)) {
                    if (1 == $filetype_forcethumb) {
                        ...
                        if ('.mp3' == $this->files[$i]['file_type']) {
                            $getID3 = new getID3();
                            $getID3->analyze($_FILES['imagefile']['tmp_name'][$i]);
                            if (isset($getID3->info['id3v2']['APIC'][0]['data'], $getID3->info['id3v2']['APIC'][0]['image_mime'])) {
                                $source_data = $getID3->info['id3v2']['APIC'][0]['data'];
                                $mime = $getID3->info['id3v2']['APIC'][0]['image_mime'];
                            } elseif (isset($getID3->info['id3v2']['PIC'][0]['data'], $getID3->info['id3v2']['PIC'][0]['image_mime'])) {
                            ...
                            if ($source_data) {
                                ...
                                } elseif (preg_match('/jpg|jpeg/', $mime)) {
                                    $ext = '.jpg';
            ...
            if ('' != $this->request['embed']) {
                ...
                if ('' != $video_id && false == strpos($video_id, '@') && false == strpos($video_id, '&')) {
  ...
  unlink($postData['oekaki']);
     ...
     * @param int    $new_w       Maximum width
     * @param int    $new_h       Maximum height
     *
    ...
    public function fastImageCopyResampled(&$dst_image, &$src_image, $dst_x, $dst_y, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h, $system, $quality = 3)
    {
        ...
        if (preg_match('/png/', $system[0]) || preg_match('/gif/', $system[0])) {
            ...
            if ($colorcount <= 256 && 0 != $colorcount) {
