...
class BoardRegenerator implements RegeneratorInterface
{
    ...
    public function regeneratePage(int $page, int $posts_per_page = 10): void
    {
        ...
        $threads = $this->entityManager->getRepository(Post::class)
     ...
     *
     * @param int $replythread The ID of the thread the header is being build for.  0 if it is for a board page
     */
     ...
     *
     * @param bool   $noboardlist   Force the board list to not be displayed
     * @param string $executiontime The time it took the page to be created
     * @param bool   $hide_extra    Hide extra footer information, and display the manage link
     */
    ...
    private function makeBoardFolders(string $directory): void
    {
        ...
        @mkdir($directory.'/thumb/', 0o777, true);
        @mkdir($directory.'/res/', 0o777, true);
    }
}
