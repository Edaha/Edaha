...
class Posting
{
    ...
    public function isReply($boardid)
    {
        ...
        if (isset($this->request['replythread'])) {
            if (0 != (int) $this->request['replythread']) {
                // Check if the thread id supplied really exists
                $thread = $this->entityManager->find(Post::class, $this->request['replythread']);
                if (null == $thread) {
    ...
    {
        if (0 == $post->board->forced_anononymous) {
     ...
     */
    public function handleTripcode(Post $post): void
    {
    ...
    public function checkPostCommands(&$postData)
    {
        ...
        if (!$postData['display_status'] && $postData['user_authority'] > 0 && 3 != $postData['user_authority']) {
    ...
    {
        $result = ['sticky' => 0, 'lock' => 0];
        if (0 == $postData['thread_info']['parent']) {
            $result = ['sticky' => $postData['sticky_on_post'], 'lock' => $postData['lock_on_post']];
        } else {
            $fields = [];
    ...
    public function checkCaptcha($board, $postData)
    {
        ...
        if (1 == $board->board_captcha) {
            ...
            if (1 == $board->board_type && $postData['is_reply']) {
     ...
     *
     * @return bool|string the path to the oekaki file if valid, false otherwise
     */
    ...
    public function doUpload(&$postData, $board)
    {
        ...
        @mkdir(KX_BOARD.'/'.$board->board_name, 0o777, true);
        @mkdir(KX_BOARD.'/'.$board->board_name.'/src/', 0o777, true);
        @mkdir(KX_BOARD.'/'.$board->board_name.'/thumb/', 0o777, true);
        @mkdir(KX_BOARD.'/'.$board->board_name.'/res/', 0o777, true);

        if ((!isset($this->request['nofile']) && 1 == $board->board_no_file) || 0 == $board->board_no_file) {
    ...
    public function parseFields()
    {
        ...
        if ('return' == $post_email || 'noko' == $post_email) {
            $post_email = '';
        }

        $post_subject = isset($this->request['subject']) ? htmlspecialchars($this->request['subject'], ENT_QUOTES) : '';

        return ['name' => $post_name, 'email' => $post_email, 'subject' => $post_subject];
    }
    ...
    public function userAuthority()
    {
        ...
        if (isset($this->request['modpassword'])) {
            ...
            if (count($results) > 0) {
                ...
                } elseif (2 == $results[0][0] && in_array($board_class->board['name'], explode('|', $results[0][1]))) {
                    $user_authority = 2; // mod
                } elseif (2 == $results[0][0] && 'allboards' == $results[0][1]) {
                    $user_authority = 2;
                }
            }
        }
