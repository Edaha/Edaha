    ...
    {% for option in options %}
        ...
        {% elseif option.config_type == "true_false" %}
          <select name="{{option.config_variable}}" id="{{option.config_variable}}">
            <option value="1"{% if option.config_variable|kxEnv == true %} selected="selected"{%endif %}>{% trans "True" %}</option>
            <option value="0"{% if option.config_variable|kxEnv == false %} selected="selected"{%endif %}>{% trans "False" %}</option>
          </select>
